...
class LitParrot(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        out, _, _, src_lens, target_lengths, flattened_targets, log_probs = self.parrot(batch)
        total_loss, code_loss, ctc_loss = self.loss_fn(out, batch, src_lens, target_lengths, flattened_targets, log_probs)
        # total_loss, code_loss = self.loss_fn(out, batch, src_lens, target_lengths, flattened_targets, log_probs)

        batch_size = batch['nam_codes'].size(0)
        self.log("train_total_loss", total_loss, prog_bar=True, batch_size=batch_size)
        self.log("train_code_loss", code_loss, prog_bar=True, batch_size=batch_size)
        self.log("train_ctc_loss", ctc_loss, prog_bar=True, batch_size=batch_size)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_total_loss", total_loss, prog_bar=True, sync_dist=True, batch_size=batch_size)
        self.log("val_code_loss", code_loss, prog_bar=True, sync_dist=True, batch_size=batch_size)
        self.log("val_ctc_loss", ctc_loss, prog_bar=True, sync_dist=True, batch_size=batch_size)
        return total_loss
...
def main(args):
    ...
    lit_parrot = LitParrot(data_config)

    # set up some model callbacks
    checkpoint_callback = ModelCheckpoint(
        save_top_k=-1,
        monitor="val_total_loss",
        filename="Seq2Seq_model-{step}-{val_total_loss_step:.2f}",
        mode="min",
